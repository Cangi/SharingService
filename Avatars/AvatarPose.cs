...
namespace SharingService.Avatars
{
   /// <summary>
    ...
    {
        private static Pose _invalidPose = new (Vector3.negativeInfinity, Quaternion.identity);
        private Pose _headPose = _invalidPose;
        private AvatarHand _leftHand = new ();
        private AvatarHand _rightHand = new ();
        private AvatarJointDescription[] _serializeJoints = Array.Empty<AvatarJointDescription>();
        ...
        /// </summary>
        public bool TryGetJoint(Handedness handedness, XRHandJointID joint, out Pose pose)
        {          
            var hand = GetHand(handedness);
            if (hand == null)
            {
        ...
        /// Try to get the shared hand 
        /// </summary>
        public bool TryGetJoint(Handedness handedness, XRHandJointID joint, out Quaternion rotation)
        {
            var hand = GetHand(handedness);
            if (hand == null)
            {
        ...
        /// </summary>
        public void SetJoint(Handedness handedness, XRHandJointID joint, Quaternion rotation)
        {
            var hand = GetHand(handedness);
            if (hand != null)
            {
        ...
        /// Set the shared hand joint position
        /// </summary>
        public void SetJoint(Handedness handedness, XRHandJointID joint, Pose pose)
        {
            var hand = GetHand(handedness);
            if (hand != null)
            {
        ...
        /// </summary>
        public bool HasJoint(Handedness handedness, XRHandJointID joint)
        {
            var hand = GetHand(handedness);
            if (hand != null)
            {
    ...
    /// </summary>
    public class AvatarPoseCacheEntry : AvatarPose, IDisposable
    {
