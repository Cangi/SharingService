...
namespace SharingService
{
    public class LogHelper<T>
    {
        ...
        private bool CanShowInformation =>
            (GlobalInformation == LogHelperState.Always && Information != LogHelperState.Never) ||
            (Information == LogHelperState.Always && GlobalInformation != LogHelperState.Never);

        private bool CanShowVerbose =>
            (GlobalVerbose == LogHelperState.Always && Verbose != LogHelperState.Never) ||
            (Verbose == LogHelperState.Always && GlobalVerbose != LogHelperState.Never);

        private bool CanShowWarning =>
            (GlobalWarning == LogHelperState.Always && Warning != LogHelperState.Never) ||
            (Warning == LogHelperState.Always && GlobalWarning != LogHelperState.Never);

        private bool CanShowError =>
            (GlobalError == LogHelperState.Always && Error != LogHelperState.Never) ||
            (Error == LogHelperState.Always && GlobalError != LogHelperState.Never);
        ...
        /// </summary>
        private void SafeLogFormat(LogType logType, LogOption logOption, object context, string format,
            params object[] args)
        {
            try
            {
            ...
            {
                Debug.LogFormat(logType, logOption, null,
                    $"[{TimeStamp()}] [{_name}] The following message was not formatted correctly (args = {(args?.Length ?? 0)})");
                Debug.Log(format);
            }
        }
