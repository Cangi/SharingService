...
namespace SharingService.Photon
{
        ...
        IDisposable
    {
            ...
            Transform origin)
        {
            ...
            _playerPose.SerializeJoints = _serializableJoints = AvatarSerialization.ExtractSerializableJoints(_profileSettings.AvatarPrefab);
            ...
            {
                PrimaryColors = profileSettings.PlayerColors
            };
        ...
        {
            if (_profileSettings.AvatarPrefab != null)
            {
                PhotonNetwork.Instantiate(
                    _profileSettings.AvatarPrefab.name,
                    Vector3.zero,
                    Quaternion.identity);
            }
        }
        ...
        {
            UpdatePlayerHand(subsystem.leftHand);
            UpdatePlayerHand(subsystem.rightHand);
        }

        private void UpdatePlayerHand(XRHand hand)
        {            
            if (hand != null)
            {
        ...
        private bool UpdatePlayerJoint(XRHand hand, AvatarJointDescription joint)
        {
            if (hand.GetJoint(joint.Joint).TryGetPose(out Pose jointPose))
            {
                if (joint.HasPose)
                {
                    _playerPose.SetJoint(hand.handedness, joint.Joint, ToPose(ToOrigin(ToMatrix(jointPose))));
                }
                else
                {
                    _playerPose.SetJoint(hand.handedness, joint.Joint, ToOrigin(jointPose.rotation));
                }
